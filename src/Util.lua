...
function GenerateQuadsFlags(atlas)
    ...
    for j = 0, 3 do
        for i = 0, 2 do
            quads[counter] = love.graphics.newQuad(x + (i * 16), y + (j * 16), 16, 16, atlas:getDimensions())
            counter = counter + 1
        end
    end
...
function GenerateQuadsPoles(atlas)
    ...
    for i = 0, 3 do
        quads[counter] = love.graphics.newQuad(x + (i * 16), y, 16, 48, atlas:getDimensions())
        counter = counter + 1
    end
